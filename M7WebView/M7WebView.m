...
#import "WKWebView+M7WebCookie.h"
#import "WKWebView+M7WebCache.h"
#import "WKWebView+LongPress.h"

#import "NSURL+M7Tool.h"
#import "M7WebView+UIDelegate.h"
...
//iPhone X：
//StatusBar高44px，NavigationBar高44px，底部TabBar高83px(49+底部距离34.f)
#define WKSTATUS_BAR_HEIGHT (iPhoneX ? 44.f : 20.f)//状态栏
#define WKNAVIGATION_BAR_HEIGHT (WKSTATUS_BAR_HEIGHT + 44.f)//导航栏(状态栏+NavBar)
#define WKTAB_BAR_HEIGHT (iPhoneX ? (49.f+34.f) : 49.f)//底部（TabBar+底部距离34.f）
...
@interface M7WebView ()<WKScriptMessageHandler,WKUIDelegate,WKNavigationDelegate,UIScrollViewDelegate>

@property (nonatomic, retain) M7WebViewMenu *menu;
...
@implementation M7WebView

+ (instancetype)shareInstance
{
    static M7WebView *baseWebview = nil;
...
{
    if (self = [super init]) {
        self.menu = [M7WebViewMenu shareInstance];
...
-(void)viewDidLayoutSubviews{
    ...
    [self.webView setFrame:CGRectMake(0, WKSTATUS_BAR_HEIGHT, self.view.frame.size.width, self.view.frame.size.height-WKSTATUS_BAR_HEIGHT)];
}
#pragma mark -
...
//        }
        _webView = [[WKWebView alloc] initWithFrame:CGRectMake(0, WKSTATUS_BAR_HEIGHT, self.view.frame.size.width, self.view.frame.size.height-WKSTATUS_BAR_HEIGHT) configuration:self.config];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message
{
    ...
    NSLog(@">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
    if (_messageHandlerDelegate && [_messageHandlerDelegate respondsToSelector:@selector(M7UserContentController:didReceiveScriptMessage:)]) {
        [_messageHandlerDelegate M7UserContentController:userContentController didReceiveScriptMessage:message];
    }
...
(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler
{
    ...
    if (![scheme containsString:@"http"] && ![scheme containsString:@"about"] && ![scheme containsString:@"file"]) {
        ...
        {
            [UIAlertController M7lertWithTitle:@"提示" message:@"是否打开appstore？" action1Title:@"返回" action2Title:@"去下载" action1:^{
                [webView goBack];
            } action2:^{
...
#pragma clang diagnostic pop
    ...
    if ([self.messageHandlerDelegate respondsToSelector:@selector(M7UserContentController:didFinishNavigation:)]) {
        [self.messageHandlerDelegate M7UserContentController:webView didFinishNavigation:navigation];
    }
}
...
- (void)webView:(WKWebView *)webView didFailProvisionalNavigation:(null_unspecified WKNavigation *)navigation withError:(NSError *)error {
    ...
    UIAlertAction *doneAction = [UIAlertAction actionWithTitle:@"刷新" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action){
        if ([self.currentURLString isEqualToString:@"about:blank"]) {
            [self loadRequestURL:self.willRequest];
        }else{
            [webView reloadFromOrigin];
        }
        
    }];
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"提示" message:@"网络加载失败" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)goback{
    [self M7RemoveScriptMessageHandlerForName];
    [self backNative];
}
        ...
        [self.menu defaultMenuShowInViewController:self title:@"更多" message:nil buttonTitleArray:buttonTitleArray buttonTitleColorArray:nil popoverPresentationControllerBlock:^(UIPopoverPresentationController * _Nonnull popover) {
        ...
        } block:^(UIAlertController * _Nonnull alertController, UIAlertAction * _Nonnull action, NSInteger buttonIndex)
         {
             if (buttonIndex == 0)
             {
                 ...
                 {
                     [UIAlertController M7lertWithTitle:@"提示" message:@"无法获取当前链接" completion:nil];
                 }
             }
             ...
             {
                 [UIAlertController M7lertWithTitle:@"提示" message:@"没写！点啥" completion:nil];
             }
...
- (UIProgressView *)wkProgressView {
    if (!_wkProgressView) {
        ...
        _wkProgressView.tintColor = [UIColor colorWithHexString:@"0b4167"];
...
- (void)setM7ProgressTintColor:(UIColor *)paprogressTintColor
{
    _paprogressTintColor = paprogressTintColor;
    self.wkProgressView.progressTintColor = paprogressTintColor;
}

- (void)setM7ProgressTrackTintColor:(UIColor *)paprogressTrackTintColor
{
...
- (void)M7RemoveScriptMessageHandlerForName{
