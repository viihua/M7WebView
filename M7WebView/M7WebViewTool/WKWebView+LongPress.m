...
//  Pkit
//
//  Created by DevMan7 on 2019/2/7.
...
#pragma clang diagnostic  ignored "-Wundeclared-selector"
         ...
         Class cls = NSClassFromString(@"M7PhotoBrowser");
...
- (void)LoadImageFromURL:(NSURL *)URLString
{
    ...
    if (!image) {
        [UIAlertController M7lertWithTitle:@"提示" message:@"下载失败" completion:nil];
        return;
    }
...
- (void)saveImageFinished:(UIImage *)image
{
    ...
    [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
        ...
        if (success){
            ...
            [UIAlertController M7lertWithTitle:@"提示" message:@"已经保存到相册" completion:nil];
        }else{
            
            [UIAlertController M7lertWithTitle:@"提示" message:@"保存失败" completion:nil];
        }
    }];
}
